...
var propTypes = {
  ...
  columnWidth: _react.PropTypes.oneOfType([_react.PropTypes.number, _react.PropTypes.string]).isRequired,
  gutterWidth: _react.PropTypes.number,
  gutterHeight: _react.PropTypes.number,
  duration: _react.PropTypes.number,
  easing: _react.PropTypes.string,
  appearDelay: _react.PropTypes.number,
  appear: _react.PropTypes.func,
  appeared: _react.PropTypes.func,
  enter: _react.PropTypes.func,
  entered: _react.PropTypes.func,
  leaved: _react.PropTypes.func,
  units: _react.PropTypes.shape({
    length: _react.PropTypes.string,
    angle: _react.PropTypes.string
  }),
  monitorImagesLoaded: _react.PropTypes.bool,
  vendorPrefix: _react.PropTypes.bool,
  userAgent: _react.PropTypes.string,
  enableSSR: _react.PropTypes.bool
};
...
var GridInline = exports.GridInline = function (_Component) {
  ...
  function GridInline(props) {
    ...
    _this.handleItemMounted = function (item) {
      var key = item.props.itemKey;

      _this.items[key] = item;
      _this.updateLayout(_this.props);

      if (_this.props.monitorImagesLoaded && typeof imagesLoaded === "function") {
        var node = _reactDom2.default.findDOMNode(item);
        var imgLoad = imagesLoaded(node);

        imgLoad.on("always", function () {
          _this.debouncedUpdateLayout(_this.props);
        });

        _this.imgLoad[key] = imgLoad;
      }
    };

    _this.handleItemUnmount = function (item) {
      var key = item.props.itemKey;
      ...
      if (_this.imgLoad.hasOwnProperty(key)) {
        _this.imgLoad[key].off("always");
        delete _this.imgLoad[key];
      }
    };

    _this.items = {};
    _this.imgLoad = {};
    _this.debouncedUpdateLayout = (0, _throttleDebounce.debounce)(20, _this.updateLayout);
  ...
  _createClass(GridInline, [{
    ...
    key: "getColumnLengthAndWidth",
    value: function getColumnLengthAndWidth(width, value, gutter) {
      if (isNumber(value)) {
        var _columnWidth = parseFloat(value, 10);

        return [Math.floor((width - (width / _columnWidth - 1) * gutter) / _columnWidth), _columnWidth];
      } else if (isPercentageNumber(value)) {
        var _columnWidth2 = width * (parseFloat(value, 10) / 100);
        var maxColumn = Math.floor(width / _columnWidth2);

        return [maxColumn, _columnWidth2 - gutter * (maxColumn - 1)];
      }
    ...
    value: function doLayout(props) {
      return _exenv2.default.canUseDOM && props.size.width !== 0 ? this.doLayoutForClient(props) : this.doLayoutForSSR(props);
    }
  }, {
    ...
    value: function doLayoutForClient(props) {
      ...
      var _getColumnLengthAndWi = this.getColumnLengthAndWidth(containerWidth, rawColumnWidth, gutterWidth),
    ...
    value: function render() {
      ...
      var _props = this.props,
          ...
          rest = _objectWithoutProperties(_props, ["gutterWidth", "gutterHeight", "columnWidth", "monitorImagesLoaded", "className", "style", "size", "component", "children", "enableSSR"]);
        ...
        validChildren.map(function (child, i) {
          return _react2.default.createElement(
            ...
            _extends({}, rest, {
              ...
              onMounted: _this3.handleItemMounted,
              onUnmount: _this3.handleItemUnmount
            }),
            child
          );
        })
...
var StackGrid = function (_Component2) {
  ...
  function StackGrid() {
    ...
    return _possibleConstructorReturn(this, (StackGrid.__proto__ || Object.getPrototypeOf(StackGrid)).apply(this, arguments));
  }

  _createClass(StackGrid, [{
    key: "render",
    value: function render() {
      _reactSizeme2.default.enableSSRBehaviour = this.props.enableSSR;
      return _react2.default.createElement(SizeAwareGridInline, this.props);
    }
  }]);
